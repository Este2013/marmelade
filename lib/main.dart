...
Future<void> main() async {
  ...
  // bindings outside the guarded zone and calling runApp inside it makes
  ...
  await runGuardedWithLogging(() async {
    ...
    log.info('image cache configured', fields: {
      'bytes': AppLog.formatBytes(_imageCacheBytes),
      'entries': _imageCacheCount,
    });
    ...
    log.info('storage resolved', fields: {
      'database': databasePath,
      'artwork': artworkDirectory.path,
    });
  ...
  Widget build(BuildContext context) {
    ...
    return DynamicColorBuilder(
      builder: (lightDynamic, darkDynamic) {
        final seed =
            darkDynamic?.primary ?? lightDynamic?.primary ?? marmeladeSeed;
        return MaterialApp(
          title: 'marmelade',
